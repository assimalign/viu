...
| **W05** | Host-neutral component-library and Browser SDK/framework segments, validated release packaging/staging, hydration foundations plus lazy activation, direct server compiler output, host-neutral SSR adaptation, SSR state round-tripping, explicit server-profile selection with reflection-free registration, installable `dotnet new` templates with an optional server host, editor hot-reload metadata, a working `dotnet watch` CSS/component path, the real-browser end-to-end harness, and live size/startup budget gates exist | Finish lazy routing, runtime inspection, connected-browser development-loop conformance, compatibility/conformance gates, and Cohesion hosting integration |
