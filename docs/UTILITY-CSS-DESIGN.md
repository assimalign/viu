...
> integration is independently published from
...
They are not retroactively redefined as the utility engine. Viu Utilities uses a separate pure core
and the independently packaged `Assimalign.Viu.UtilityCss.Build` MSBuild host. No Viu SDK or
framework references either UtilityCss package.
...
## 7. Compilation and standalone MSBuild delivery
...
The `Assimalign.Viu.UtilityCss.Build` package supplies `ViuGenerateUtilityCss` through a
`tasks/netstandard2.0` payload. It calls the same `Assimalign.Viu.UtilityCss` compiler as
programmatic consumers and writes:

```text
obj/<configuration>/<tfm>/utilitycss/<AssemblyName>.utilities.css
```

NuGet-imported targets then:

- discover `.viu`, `.vue`, `.razor`, `.cshtml`, `.html`, and `.htm` sources;
- slice `.viu` and `.vue` inputs to template content before candidate scanning;
- compare exact generated bytes and avoid touching the file on a semantic no-op;
- register the stylesheet through paired `DefineStaticWebAssets` and
  `DefineStaticWebAssetEndpoints` calls when the host uses Static Web Assets; or
- add the stylesheet to the ordinary content output pipeline for plain project hosts.

The package never rewrites or injects a host page; the host owns its HTML link. Referencing the build
package is the opt-in for any MSBuild project type, with no Viu SDK, framework, runtime, or Node
dependency.

Package-consumer verification is distinct from in-repo tests: pack to `_out/packages`, build a
consumer from the installed package boundary, confirm the Static Web Asset plus endpoint pair or
plain content copy, and verify a no-change rebuild preserves the output timestamp.
