...
The watch-list task allows five seconds for worker readiness. On Linux, readiness and reuse
validate the process identifier together with the kernel start-clock token from `/proc/<pid>/stat`;
independently observed wall-clock process start times are not identical. The existing `worker-start`
field remains available to other state-file consumers. This corrects the Linux readiness failure
tracked by [V01.01.12.05.03] ([#370](https://github.com/assimalign/viu/issues/370)).
