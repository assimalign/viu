...
The watch-list task allows five seconds for worker readiness. On Linux, the task and RunHost's
existing-worker check validate the process identifier together with the kernel start-clock token from
`/proc/<pid>/stat`; independently observed wall-clock process start times are not identical. RunHost
therefore preserves the watch-owned refresh topology instead of adding its Visual Studio bridge to a
`dotnet watch` child. The existing `worker-start` field remains available to other state-file
consumers. This corrects the Linux readiness and browser-refresh failures tracked by
[V01.01.12.05.03] ([#370](https://github.com/assimalign/viu/issues/370)).

Watch-owned worker output is retained beside `ViuCssHotReloadStateFile` in
`worker.state.stdout.log` and `worker.state.stderr.log` by default. These files are reset for each
launch and remain writable after the watch-list MSBuild process exits. A timeout or early exit
reports the exact polled state path and bounded tails of each output stream, including .NET host
startup output, in the build error. The task stops its uninitialized child on timeout. These logs
are development diagnostics and are never published; RunHost-owned workers retain their existing
standard-output update protocol.
...
bridge. The bridge forwards the page's `Origin` header on its upstream websocket handshake so the
refresh server applies its normal browser-origin validation to the relayed connection. RunHost can
also send the client's existing targeted `UpdateStaticFile` message locally when the worker
regenerates a registered stylesheet. Each page or tab also receives one idempotent
