...
.DESCRIPTION
    The reliable, multi-step intake path used by the `vuecs-work-items` skill:
...
.EXAMPLE
    ...
    ./New-VuecsWorkItem.ps1 -Search "keyed diff lis moves"

.EXAMPLE
    # Scope-creep TASK discovered on feature branch feature/V01.01.02.01-dependency-engine.
    ./New-VuecsWorkItem.ps1 -As task -Title "Guard against reentrant trigger during effect cleanup" `
...
.EXAMPLE
    ...
    ./New-VuecsWorkItem.ps1 -As feature -Title "Implement readonly shallow reactive views" -Wave W02

.EXAMPLE
    # Assemble the grouped "Closes #N" block (planned vs discovered) for the current branch's PR body.
    ./New-VuecsWorkItem.ps1 -EmitClosesBlock
#>
...
$ProjectNum  = 15
$WbsPattern  = 'V\d{2}(?:\.\d{2})+'      # vuecs WBS codes are V-prefixed
$AreaPattern = 'Framework - (.+)$'       # area-epic title convention
$ManifestDir = 'vuecs'                   # per-branch manifest folder under .git/
$IssueLimit  = 5000
...
function New-BodyTemplate {
    ...
    else { [void]$sb.AppendLine('- <Vue 3 API/semantics parity reference (vuejs.org / vuejs/core), the WHATWG/W3C spec for DOM behavior, or a note that it is a Vuecs runtime-contract concern>') }
    return $sb.ToString()
}
