---
name: vuecs-work-items
description: Create and link GitHub work items (area epics / features / tasks) in the assimalign/vuecs Project #15 using the gh CLI — following the V01.01.NN WBS scheme, native parent/sub-issue links, and the Summary / Acceptance Criteria body template. Use whenever new development is requested, and ESPECIALLY when scope creep is discovered mid-branch: file the out-of-scope work as its own tracked work item so one PR can attach and close several items. Triggers include "create a work item", "file an issue for this", "capture this scope creep", "track this extra change", "open a Vuecs issue", "this is out of scope — log it", or "assemble the Closes list for my PR". Use only in the assimalign/vuecs repo.
---

# Vuecs Work Items

Capture development as tracked GitHub work items in **Project #15 "Vuecs"** under the `assimalign` org,
...
The reliable end-to-end path is the script [scripts/New-VuecsWorkItem.ps1](scripts/New-VuecsWorkItem.ps1).

> Note: Project #15 is vuecs-only (406 stray `assimalign/cohesion` items were removed on 2026-07-16).
...
The program root is `[V01.01.00] Vuecs - Framework Libraries`. Your branch names the feature in flight:
...
```pwsh
.claude/skills/vuecs-work-items/scripts/New-VuecsWorkItem.ps1 -Search "keyed diff lis moves"
```
...
.claude/skills/vuecs-work-items/scripts/New-VuecsWorkItem.ps1 -As task `
...
.claude/skills/vuecs-work-items/scripts/New-VuecsWorkItem.ps1 -As feature `
...
.claude/skills/vuecs-work-items/scripts/New-VuecsWorkItem.ps1 -Parent V01.01.11 `
...
.claude/skills/vuecs-work-items/scripts/New-VuecsWorkItem.ps1 -Title "..." -DryRun
```
...
in a per-branch manifest under `.git/vuecs/`; and **backs off and retries on GitHub rate limits**
...
cite the WHATWG/W3C spec; otherwise note that it is a Vuecs runtime-contract concern.
...
(the manifest lives in `.git/vuecs/`, so it is per-worktree):
