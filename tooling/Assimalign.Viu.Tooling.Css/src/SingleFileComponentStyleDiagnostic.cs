...
/// style-block rewrite (today: a malformed <c>v-bind()</c> — an unterminated <c>v-bind(</c> or an
/// empty <c>v-bind()</c> — [V01.01.06.06]). It carries the base <see cref="Diagnostic"/> exactly as the CSS
/// rewriter reported it (positions relative to the style block's content) together with
...
/// <c>ViuBundleCss</c> task host does not report these (the generator already does — the task is the
...
/// <param name="BlockContentStart">The file position where the originating style block's content begins.</param>
