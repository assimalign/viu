...
/// </summary>
/// <remarks>
/// The <see cref="PatchFlag"/> and <see cref="DynamicProperties"/> fields are populated by property analysis
/// ([V01.01.05.03]) and the element-level patch-flag inference and block-emission decisions ([V01.01.05.06]);
/// <see cref="IsBlock"/> and <see cref="DisableTracking"/> record whether this virtual node opens an optimization
...
/// </remarks>
public sealed record VirtualNodeCall : TemplateSyntaxNode
{
    /// <summary>
    /// The virtual-node type: an element tag <see cref="string"/> (already quoted), a <see cref="RuntimeHelper"/>
    /// (e.g. <c>Fragment</c>/<c>Teleport</c>), or a <see cref="CallExpression"/>
    /// (<c>resolveDynamicComponent(...)</c>).
    /// </summary>
    public required object Tag { get; init; }

    /// <summary>The properties expression (<see cref="ObjectExpression"/>/<see cref="CallExpression"/>/expression), or <see langword="null"/>.</summary>
    ...
    /// <summary>
    /// The dynamic property names list: a stringified array literal or a <see cref="SimpleExpressionNode"/>, or
    /// <see langword="null"/> when the patch flag alone identifies what may change.
    /// </summary>
    public object? DynamicProperties { get; init; }
    ...
    /// <summary>Whether this virtual node opens an optimization block.</summary>
    ...
    /// <summary>Whether the virtual node is a component (selects the <c>createVNode</c> vs <c>createElementVNode</c> helper).</summary>
    public bool IsComponent { get; init; }

    /// <inheritdoc />
    public override NodeType NodeType => NodeType.VirtualNodeCall;
}
