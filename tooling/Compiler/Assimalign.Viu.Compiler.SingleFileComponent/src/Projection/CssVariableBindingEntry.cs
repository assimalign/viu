...
/// original C# <see cref="Expression"/>. Runtime CSS-variable application remains deferred under <c>[STY-6]</c>, so the source emitter deliberately publishes no runtime seam
...
public readonly record struct CssVariableBindingEntry(string Name, string Expression);
