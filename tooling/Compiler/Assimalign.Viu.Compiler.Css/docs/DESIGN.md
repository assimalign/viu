...
The `.viu` style compilation — parse the component, run the CSS-Modules / `v-bind()` rewrites,
...
| `CssComponentHash` | The eight-digit FNV-1a hash of the project-relative path, used only as the salt for stable module and binding names. |
| `SingleFileComponentStyleCompiler` | The compilation itself: `Compile(parse, localHashSalt)` (generator, which already holds the parse) and `CompileFile(parser, text, path, projectDirectory)` (task). Returns a `SingleFileComponentStyleCompilation`. |
...
- **Roslyn never ships beside the task** (decision [V01.01.12.19.04], #279). The Roslyn dependency this
  ...
  never dispatch template content, and the `v-bind()`/`module` rewrites live in
...
Every newline the bundler emits is `\n`; the compiled CSS segments are LF for module / `v-bind()`
