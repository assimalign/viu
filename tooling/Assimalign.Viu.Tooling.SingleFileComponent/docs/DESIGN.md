# Assimalign.Viu.Tooling.SingleFileComponent — design
...
2. the `Assimalign.Viu.Tooling.LanguageService` (shipped in the language-server payload of the Visual Studio
...
- **All-internal surface.** The facade's result exposes the whole evolving model graph, so a public
  ...
  assemblies), matching the `Assimalign.Viu.Tooling.Css` → generator precedent.
- **Packaging.** The assembly joins the generator's computed analyzer closure (packed at
  ...
  `Tooling.Css` graph, whose output directory sweeps wholesale into the SDK package's `Tasks/`
