...
public sealed class RoutedSsrHydrationIntegrationTests
{
    ...
    {
        Scheduler.Reset();
        // Preserve the single-event-loop contract: queued lifecycle work must not run concurrently
        // with the synchronous hydration walk on the multithreaded test host.
        using TestSchedulerPump schedulerPump = TestSchedulerPump.Install();
