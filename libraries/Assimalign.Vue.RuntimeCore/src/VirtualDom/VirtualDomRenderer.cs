...
public sealed class VirtualDomRenderer<TNode>
{
    ...
    public void Render(TNode container, VNode tree)
    {
    ...
    private MountedNode PatchNode(TNode parent, MountedNode current, VNode next)
    {
        ...
        switch (current)
        {
            case MountedTextNode currentTextNode when next is VText nextText:
                var currentText = (VText)currentTextNode.Node;
            ...
            case MountedElementNode currentElementNode when next is VElement nextElement:
                PatchProperties(currentElementNode.NativeNode, (VElement)currentElementNode.Node, nextElement);
            ...
            case MountedFragmentNode currentFragmentNode when next is VFragment nextFragment:
                PatchChildren(
                    parent,
    ...
    private void PatchProperties(TNode nativeNode, VElement current, VElement next)
    {
    ...
    private MountedNode Mount(VNode node)
    {
        switch (node)
        {
            case VText text:
    ...
    {
        protected MountedNode(VNode node)
        {
            Node = node;
        }
    ...
    {
        public MountedTextNode(VNode node, TNode nativeNode)
            : base(node)
        {
    ...
    {
        public MountedElementNode(VNode node, TNode nativeNode, List<MountedNode> children)
            : base(node)
        {
            NativeNode = nativeNode;
            Children = children;
    ...
    {
        public MountedFragmentNode(VNode node, TNode startMarker, TNode endMarker, List<MountedNode> children)
            : base(node)
        {
