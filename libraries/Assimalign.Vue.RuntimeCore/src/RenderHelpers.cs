...
/// <c>using static global::Assimalign.Vue.RuntimeCore.RenderHelpers;</c> — the C# analogue of upstream's
...
/// is <c>Assimalign.Vue.Syntax.Templates/docs/DESIGN.md</c>; this surface satisfies every call shape the
...
/// </para>
/// <para>
/// This class references only <c>Assimalign.Vue.RuntimeCore</c>, <c>Assimalign.Vue.Reactivity</c>, and
/// <c>Assimalign.Vue.Shared</c> — never any <c>Assimalign.Vue.Syntax.*</c> assembly. The contract flows one
...
/// <c>_TransitionGroup</c>) are <b>not</b> here: their behavior lives in <c>Assimalign.Vue.RuntimeDom</c>,
...
public static class RenderHelpers
{
    ...
    /// <c>guardReactiveProps</c>). Vuecs prop bags are not identity-swapping proxies, so this returns the
    ...
    /// <c>Assimalign.Vue.Reactivity</c> (upstream: <c>unref</c>). Reading a ref's value tracks it.
    /// </summary>
    ...
    // ==== Vuecs-defined helpers (no upstream counterpart; see docs/DESIGN.md divergence table) ===

    /// <summary>
    /// Builds a props/slots object from name/value tuples (Vuecs-defined). JavaScript object literals
    ...
    /// <summary>
    /// Target-types an event-handler value expression (Vuecs-defined). A C# lambda or method group has no
    ...
    /// <summary>Target-types a void inline or method-group handler taking the event argument (Vuecs-defined).</summary>
    /// <param name="handler">The handler.</param>
    public static object? _withHandler(Action<object?> handler) => handler;

    /// <summary>Target-types a parameterless void method-group handler (Vuecs-defined).</summary>
    /// <param name="handler">The handler.</param>
    public static object? _withHandler(Action handler) => handler;

    /// <summary>Target-types a parameterless value method-group handler (Vuecs-defined).</summary>
    /// <param name="handler">The handler.</param>
    public static object? _withHandler(Func<object?> handler) => handler;

    /// <summary>Target-types any other delegate-shaped handler (Vuecs-defined catch-all for method groups).</summary>
    /// <param name="handler">The handler.</param>
    public static object? _withHandler(Delegate handler) => handler;

    /// <summary>
    /// Completes a <c>v-once</c> cache write (Vuecs-defined). Upstream's comma sequence
    ...
    /// <summary>
    /// Clones a cached array so a block's children are a fresh array on reuse (Vuecs-defined). Upstream's
