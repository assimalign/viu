...
internal sealed class AccessorReference<T> : ReactiveValue<T>
{
    ...
    /// <summary>Creates a reference projecting through <paramref name="getter"/> and <paramref name="setter"/>.</summary>
    /// <param name="getter">Invoked on every read; its reactive reads establish the reference's dependencies.</param>
    /// <param name="setter">Invoked on every write, or <see langword="null"/> for a read-only reference.</param>
    ...
    public override T Value
    {
        ...
        set
        {
            if (_setter is null)
            {
                RuntimeWarnings.Warn("Write operation failed: this reference is readonly (it was created without a setter).");
                return;
            }
            _setter(value);
        }
