# Assimalign.Vue.Syntax.Css — overview

The build-time CSS language area of the `Assimalign.Vue.Syntax.*` cluster: it tokenizes and rule-parses
...
- **`CssScopedRewriter.Rewrite(stylesheet, scopeId)`** — the scoped transform, returning deterministic
  scoped CSS text.
- **`CssError` / `CssErrorCode`** — the Vuecs-defined recoverable-diagnostic catalog (2000-based).

## Boundary

Roots on `Assimalign.Vue.Syntax` only. It never references the `SingleFileComponent` parser or any other
