...
`Assimalign.Vue.SingleFileComponent` defines and parses the `.viu` single-file component (SFC) format — the Vuecs
...
suite (`libraries/Assimalign.Vue.SingleFileComponent/test/`) pins every rule stated here.
...
The diagnostic codes (`SingleFileComponentErrorCode`) are **Vuecs-defined**. Unlike `Assimalign.Vue.Compiler`'s
