...
internal static class TransformText
{
    ...
    private static List<TemplateSyntaxNode>? ResolveChildren(TemplateSyntaxNode node, TransformContext context) => node switch
    {
    ...
    private static bool IsText(TemplateSyntaxNode node) => node is TextNode or InterpolationNode;

    private static bool IsFastPathTextParent(TemplateSyntaxNode node, TransformContext context)
    {
        if (node is RootNode)
        {
            return true;
