...
internal static class UtilityDefaultThemeCatalog
{
    // Compatibility token values are stored as a transformed, independently consumed data vector.
    // Tailwind CSS is copyright Tailwind Labs and licensed under MIT. Provenance and license text:
...
""";
    ...
    public static UtilityTheme CreateTheme() =>
        new(
            CreateProperties(),
            null,
            CreateKeyframes());

    private static IEnumerable<UtilityThemeProperty> CreateProperties()
    {
        var lines = PropertyData.Split(
            new[] { '\n' },
        ...
        foreach (var sourceLine in lines)
        {
            ...
            if (separator <= 0 || separator == line.Length - 1)
            {
                throw new InvalidOperationException(
                    "The built-in utility theme data contains a malformed property.");
            }

            yield return new UtilityThemeProperty(
                line.Substring(0, separator),
                line.Substring(separator + 1),
                UtilityThemeOptions.Default);
        }

        const UtilityThemeOptions deprecatedOptions =
            UtilityThemeOptions.Default |
            UtilityThemeOptions.Inline |
            UtilityThemeOptions.Reference;
        yield return new UtilityThemeProperty(
            "--blur",
            "8px",
            deprecatedOptions);
    ...
    private static IEnumerable<UtilityThemeKeyframe> CreateKeyframes()
    {
        yield return new UtilityThemeKeyframe(
            "bounce",
