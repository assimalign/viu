...
// This framing code deliberately follows Assimalign.Viu.LanguageServer's proven stdio pattern.
// A shared protocol transport library is deferred until both servers' boundaries have stabilized.
internal sealed class LanguageServerProtocolMessageReader
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            if (count == 0)
            {
                ...
                throw new EndOfStreamException(
                    "The language-server message header ended unexpectedly.");
            }
            ...
            if (header.Length > MaximumHeaderLength)
            {
                throw new InvalidDataException(
                    "The language-server message header is too large.");
            }

            terminatorMatchLength = AdvanceHeaderTerminator(
                next[0],
                terminatorMatchLength);
        ...
        {
            throw new InvalidDataException(
                "The language-server message payload is too large.");
        }
    ...
    private static int AdvanceHeaderTerminator(
        byte value,
