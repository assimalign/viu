// netstandard2.0 shims enabling the C# `required` member feature used by the immutable AST node
// records. These attributes ship in the BCL only from net7.0 onward; on the netstandard2.0 analyzer
// surface the compiler still recognises them by full name, so declaring them here lights up `required`
// without any runtime dependency. Referenced only by the C# compiler; never used at run time.
