...
public sealed class StyleAndClassNormalizationCompatibilityTests
{
    [Fact]
    ...
    {
        NameNormalization.Hyphenate("backgroundColor").ShouldBe("background-color");
        NameNormalization.Hyphenate("color").ShouldBe("color");
        // No hyphen before a leading capital, so a vendor prefix survives intact.
        NameNormalization.Hyphenate("WebkitTransition").ShouldBe("webkit-transition");
        NameNormalization.Hyphenate("ArrowUp").ShouldBe("arrow-up");
    }
