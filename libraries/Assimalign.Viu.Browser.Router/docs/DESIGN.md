...
`RouterLink`'s guard reads a `RouterLinkClickEvent` — a DOM-free carrier for the button and modifier
state of a click — so the Router components run against the in-memory Testing renderer
...
runtime taking on a specific feature library, which is the dependency direction backwards.
So the DOM runtime stays Router-agnostic behind a generic seam, and the concrete mapping
...
conversion. It is the same shape the Testing renderer's `Action<object?>` dispatch
(`TestEventDispatcher`) uses: same handler shape, host-specific payload.
`RouterLinkDomBridge.Install()` sets that invoker; with
...
- **Already-prevented events fall through.** The dispatch payload carries the live event's
  ...
  guard bails — and the bridge does not re-signal, because the browser already suppressed the default
  and an outer handler that already claimed the click wins.
- **A new prevent re-crosses the boundary once.** When the guard intercepts an unmodified
  primary-button click it calls `PreventDefault()` on the `RouterLinkClickEvent`; the bridge
  propagates that to `BrowserEvent.PreventDefault()`, whose response flag rides the single synchronous dispatch
