...
internal interface IBrowserHistoryInterop
{
    ...
    /// The history instance whose saved-position and current-position bookkeeping must advance.
    /// </param>
    void Push(
    ...
    /// <param name="subscriptionIdentifier">
    /// The history instance whose current-position bookkeeping must be replaced.
    /// </param>
    void Replace(int subscriptionIdentifier, string toUrl, RouterHistoryState newState);
    ...
    /// <summary>
    /// Applies one absolute or selector-derived target in a single interop crossing after rendering.
    /// </summary>
    /// <param name="target">The validated host-free scroll target.</param>
    void Scroll(ScrollTarget target);
    ...
    /// <returns>
    /// The instance-specific subscription identifier that every subsequent mutation must pass back.
    /// </returns>
    int Subscribe(Action<BrowserHistorySnapshot> onPopState);
    ...
    /// </summary>
    /// <param name="subscriptionIdentifier">The identifier returned by <see cref="Subscribe"/>.</param>
    void Unsubscribe(int subscriptionIdentifier);
}
