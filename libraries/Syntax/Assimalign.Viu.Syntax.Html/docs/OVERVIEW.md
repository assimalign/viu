...
The publicly consumable `.html` language of the `Assimalign.Viu.Syntax.*` cluster — the parser Viu
build tooling or developer-authored composition roots register for HTML documents, above all the
WASM host page (`wwwroot/index.html`) whose entry references the build rewrites. Viu *template*
markup is the separate `Assimalign.Viu.Syntax.Templates` parser, not this one. Area: `V01.01.05`
(Syntax cluster).
...
- **`HtmlSyntaxParser`** — the `SyntaxParser<HtmlSyntaxNode>` a composition root registers for
  `.html` sources.
...
- Roots on **`Assimalign.Viu.Syntax`** only; a build-time library on the netstandard2.0 analyzer TFM
  that runs inside Roslyn generator hosts or other build/editor-time processes.
