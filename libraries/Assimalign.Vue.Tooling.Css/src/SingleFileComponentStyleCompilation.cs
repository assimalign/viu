...
/// build-time hosts consume ([V01.01.12.12]). The <c>Assimalign.Vue.Syntax.Generators</c> generator maps it
/// into its cached model to emit the <c>ScopeId</c>/<c>ExtractedStyles</c> constants, the <c>$style</c>
/// accessors, and the <c>v-bind()</c> seam; the <c>VuecsBundleCss</c> MSBuild task consumes only
