...
/// <para>
/// A base <b>class</b> (not an interface) is deliberate: <c>Reactive.IsRef</c>/<c>Unref</c>-style
/// introspection is an O(1), reflection-free <c>is <see cref="ReactiveValue"/></c> type check, and
/// the shared dependency cell lives on the base as a field (a direct load on the hot path) rather
/// than behind an interface-property stub. The base cannot be constructed or subclassed outside this
/// assembly (its constructor is <see langword="private protected"/>), so the reactive value set stays
/// closed. Not thread-safe: designed for the single-threaded JS event-loop model.
/// </para>
/// </summary>
public abstract class ReactiveValue
{
    /// <summary>
