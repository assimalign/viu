...
/// <para>
/// The proof lives in its own project (nested under Core <c>test/</c>, mirroring
/// <c>Assimalign.Viu.Shared.GeneratorFixture</c>) so the generator's transitive
/// <c>Assimalign.Viu.Syntax.Templates</c> reference — which links copies of Shared's
/// <c>PatchFlags</c>/<c>SlotFlags</c> — never collides with <c>Shared.dll</c> in the main
/// <c>Core.Tests</c> compile.
/// </para>
/// </summary>
public sealed class CompiledRenderEndToEndTests : IClassFixture<CompiledRenderEndToEndTests.CompiledCounter>, IDisposable
{
    ...
    public sealed class CompiledCounter
    {
        ...
        private const string HandWrittenHalf =
            ...
            "using System.Collections.Generic;\n" +
            "using Assimalign.Viu;\n" +
            "using Assimalign.Viu.CompiledRenderTests;\n" +
            "namespace Demo\n" +
            "{\n" +
...
internal static class CompiledRenderSupport
{
    ...
    private static IReadOnlyList<MetadataReference> ResolveReferences()
    {
        ...
        // The framework's trusted platform assemblies plus exactly the Viu assemblies the generated
        // render and its hand-written half bind against — the runtime helper surface (Core),
