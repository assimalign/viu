...
public sealed class RefTests
{
    ...
    public void SettingNaNOverNaNDoesNotTrigger()
    {
        ...
        var number = Reactive.Ref(double.NaN);
        var runs = 0;
        Reactive.Effect(() =>
        {
            runs++;
    ...
    {
        var list = Reactive.ShallowRef(new List<int> { 1 });
        var runs = 0;
        Reactive.Effect(() =>
        {
            runs++;
    ...
    public void CustomRefControlsItsOwnTrackAndTrigger()
    {
        ...
        Action? flush = null;
        var backing = 0;
        var debounced = Reactive.CustomRef<int>((track, trigger) => (
            Get: () =>
            {
                track();
                return backing;
    ...
    {
        IRef boxedRef = Reactive.Ref(3);
        boxedRef.Value.ShouldBe(3);

        IRef boxedComputed = Reactive.Computed(() => 9);
