...
/// ref's dep and returns the getter/setter; the ref performs no automatic tracking, triggering, or
/// change detection of its own. Not thread-safe (single-threaded JS event-loop model).
/// </summary>
/// <typeparam name="T">The type of the contained value.</typeparam>
public sealed class CustomRef<T> : IRef<T>, ITrackedRef
{
    ...
    public CustomRef(CustomRefFactory<T> factory)
    {
        ArgumentNullException.ThrowIfNull(factory);
        var (get, set) = factory(_dep.Track, _dep.Trigger);
