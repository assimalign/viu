...
internal static class ReactivityState
{
    ...
    internal static ISubscriber? ActiveSub;
    ...
    private static ISubscriber? _batchedSub;
    ...
    internal static void Batch(ISubscriber sub, bool isComputed)
    {
        sub.Flags |= SubscriberFlags.Notified;
        if (isComputed)
        {
            sub.NextBatched = _batchedComputed;
            _batchedComputed = sub;
            return;
        }
        sub.NextBatched = _batchedSub;
        _batchedSub = sub;
    }
    ...
    internal static void EndBatch()
    {
        ...
        {
            var c = _batchedComputed;
            _batchedComputed = null;
            while (c is not null)
            {
                var next = c.NextBatched;
                c.NextBatched = null;
                c.Flags &= ~SubscriberFlags.Notified;
                c = next;
            }
        }
        ExceptionDispatchInfo? error = null;
        while (_batchedSub is not null)
        {
            var e = _batchedSub;
            _batchedSub = null;
            while (e is not null)
            {
                var next = e.NextBatched;
                e.NextBatched = null;
                e.Flags &= ~SubscriberFlags.Notified;
                if ((e.Flags & SubscriberFlags.Active) != 0)
                {
                    try
                    {
                        ((ReactiveEffect)e).TriggerInvalidation();
                    }
                    catch (Exception ex)
                    {
