...
/// <summary>
/// The frozen DOM knowledge tables: which tag names the platform defines, which attributes are
/// boolean, and which property names carry a casing exception. The template compiler resolves native
...
/// their specs.
/// </summary>
internal static class DomKnowledge
{
    ...
    /// <summary>Whether <paramref name="attributeName"/> is a boolean attribute (its presence means true; its value is ignored).</summary>
    ...
    /// containing <c>&gt;</c>, <c>/</c>, <c>=</c>, quotes, or whitespace/control characters are
    /// excluded rather than escaped, because an attribute <em>name</em> has no escaping form that
    /// keeps the serialized markup unambiguous.
    /// </summary>
    /// <param name="attributeName">The attribute name.</param>
    ...
    /// <summary>
    /// Maps a camelCase property name to its attribute name, covering the casing exceptions where
    /// the DOM property name and the HTML attribute name differ (<c>className</c> → <c>class</c>,
