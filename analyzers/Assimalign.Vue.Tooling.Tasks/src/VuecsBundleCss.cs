...
namespace Assimalign.Vue.Tooling.Tasks;
...
public sealed class VuecsBundleCss : Microsoft.Build.Utilities.Task, ICancelableTask
{
    private readonly CancellationTokenSource _cancellation = new CancellationTokenSource();

    /// <summary>
    /// The consuming project's <c>.viu</c> single-file components — the same set flowed to the generator as
    /// <c>AdditionalFiles</c> (<c>@(VuecsSingleFileComponent)</c>). Each item's <c>FullPath</c> is read and
    ...
    /// (e.g. <c>obj/&lt;config&gt;/&lt;tfm&gt;/vuecs/&lt;name&gt;.vuecs.css</c>). The directory is created if
    /// it does not exist.
    /// </summary>
    [Required]
    ...
    public override bool Execute()
    {
        ...
        if (bundle is null)
        {
            ...
            BundlePath = string.Empty;
            BundleWritten = false;
            Log.LogMessage(MessageImportance.Low, "VuecsBundleCss: no @style blocks found; no bundle written.");
            return true;
        }
        ...
        try
        {
                ...
                string.Equals(File.ReadAllText(BundleOutputPath), bundle, StringComparison.Ordinal))
            {
                ...
                BundleWritten = false;
                Log.LogMessage(MessageImportance.Low, "VuecsBundleCss: bundle unchanged; skipped write of {0}.", BundleOutputPath);
                return true;
            }
            ...
            BundlePath = BundleOutputPath;
            BundleWritten = true;
            Log.LogMessage(MessageImportance.Normal, "VuecsBundleCss: wrote component style bundle to {0}.", BundleOutputPath);
            return true;
        }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
